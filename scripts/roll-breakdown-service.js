...
function isAutomaticRollTableMessage(message) {
  return Boolean(message?.flags?.["core.RollTable"]);
}
...
export class RollBreakdownService {
  ...
  async enhanceChatMessage(message, html) {
    ...
    if (isAutomaticRollTableMessage(message)) {
      this.log("Skipping message because it was generated by a roll table.", { messageId: message?.id });
      return;
    }
